pipeline {
    ...
    stages {
        ...
        stage('Setup Python Environment in WSL') {
            steps {
                script {
                    bat 'wsl -d Ubuntu python3 -m venv /home/vedang/ansible_venv || echo "Virtual environment creation SUCCESS"'
                    bat 'wsl -d Ubuntu /home/vedang/ansible_venv/bin/pip install docker ansible || echo "Module installation SUCCESS"'
                    bat 'wsl -d Ubuntu /home/vedang/ansible_venv/bin/ansible-galaxy collection install community.docker || echo "Collection installation SUCCESS"'
                }
            }
        }
